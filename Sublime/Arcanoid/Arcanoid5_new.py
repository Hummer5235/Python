...
BONUS_HEAL = None # Бонус здоровье
			...
			if len(spriteHitList) > 0:
				for sprite in spriteHitList:
					...
					if sprite.name == "BLOCK":
						...
						# Выдать бонус случайным образом
						if True:
							newBONUS_HEAL = bonus(self.rect.x,self.rect.y)
							BONUS_LIST.append(newBONUS_HEAL)
					elif sprite.name =="PLATFORM":
						BS = BALLSPEED
						if spriteRight-BS<self.rect.left:
							self.speedx *= -1
						elif spriteLeft+BS>self.rect.right:
							self.speedx *= -1
						elif spriteTop+BS>self.rect.bottom:
							self.speedy *= -1
...
class Platform(pg.sprite.Sprite):
	...
	def update(self,keys,platform,blocks,ball,*args):  # Управление платформой
		
		if keys[K_a] and self.rect.left>0:            # Добавить границы
			self.rect.x -= 10 
		if keys[K_d] and self.rect.right<800:
			self.rect.x += 10

		hitGroup=pg.sprite.Group(BONUS_LIST) #Группа спрайтов
		spriteHitList=pg.sprite.spritecollide(self,hitGroup,False) #Список колизий

		if len(spriteHitList) > 0:
			for sprite in spriteHitList:
				if sprite.name == "HEAL":
					BONUS_LIST.pop(BONUS_LIST.index(sprite))
					sprite.kill()
					game.ball.lifes+=1
...
class Game:
	...
	def display_frame(self,screen,keys):
		...
		self.sprites.update(keys,self.platform,self.blocks,0)
