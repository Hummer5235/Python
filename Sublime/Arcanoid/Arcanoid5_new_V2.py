			...
			if len(spriteHitList) > 0:
				for sprite in spriteHitList:
					if sprite.name == "BLOCK":
						...
						if r==11:
							newBonus = bonus("HEAL",self.rect.x,self.rect.y,"heal.png")
							BONUS_LIST.append(newBonus)

						if r2==5:
							newBonus = bonus("BOMB",self.rect.x,self.rect.y,"bomb.png")
							BONUS_LIST.append(newBonus)
...
class Platform(pg.sprite.Sprite):
	...
	def update(self,keys,blocks,*args):  # Управление платформой
		...
		# Проверка на взаимодействие с бонусами
...
class Game:
	...
	def display_frame(self,screen,keys):
		...
		#Проверка на победу
		if self.ball.score == 2:
			lifes = self.font2.render("WINNER!!!!",1,(0,255,0))
			screen.blit(lifes,(230,250))
			self.ball.speedy= 0
			self.ball.speedx= 0
