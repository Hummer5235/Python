...
@router.message(F.text == stats_game_button.text,IsPlayer(users_in_game))
async def process_stat_command(message:Message,bot):
    ...
    bot_msg = await message.answer(
        f'Статистика игр:\n'
        f'Всего: {users[user_id]["total_games"]}\n'
...
async def process_generate_question(message,user_id):
    ...
    question_message_text = [f'В какой день <b>{users[user_id]["secret_month"].capitalize()}</b> празднуется:\n'
        f'<b>{users[user_id]["event"]}</b>']


    #Если это не первый вопрос, тогда добавляется ожидание 2 секунды
    #await asyncio.sleep(2)
    # if message.text != guess_date_button.text:
    #     bot_msg = await message.edit_text(
...
async def process_numbers_answer(callback:CallbackQuery,bot):
    user = callback.from_user
    user_id = user.id
    ...
    if users[user_id]['in_game']:
        if users[user_id]['secret_day'] == day_answer:
            ...
            await callback.answer(
                'Ура!!! Вы угадали день!\n\n'
                f'{users[user_id]["secret_day"]} {users[user_id]["secret_month"].capitalize()} празднуется:\n'
