...
welcome_list = ['привет','hi','hello','здравствуй','здравствуйте','прив']


def write_table_data(user):
    global name,surname,age
    with sqlite3.connect('db/database.db') as db:
        cursor = db.cursor()
        query = ''' CREATE TABLE IF NOT EXISTS Users(user_id INTEGER, username TEXT, name TEXT, surname TEXT,
        age INTEGER, UNIQUE(user_id))'''
        cursor.execute(query)
        query = ''' INSERT INTO Users(user_id,username,name,surname,age) VALUES(?,?,?,?,?) '''
        try:
            cursor.executemany(query, [(user.id, user.username,name,surname,age)])
            return True,name
        except:
            print('Ошибка')
            return False, False
...
def get_table_data(user):
    ...
    with sqlite3.connect('db/database.db') as db:
        ...
        age INTEGER, UNIQUE(user_id))'''
        cursor.execute(query)
        query = ''' SELECT username,name,surname,age FROM Users WHERE user_id = ?'''
        ...
        if len(cursor) == 0:
            ...
            return user_list
        # else:
        #     bot.send_message(user.id, f'{name} {surname}, Вы успешно зарегистрированы!')
        # return


# get_table_data(123231,'bot')
...
def start(message):
    ...
    elif message.text.lower() in welcome_list:
        user_list = get_table_data(message.from_user)
        if not user_list:
            bot.send_message(message.from_user.id, 'Нажмите "Регистрация"', reply_markup=keyboard)
        else:
            bot.send_message(message.from_user.id, f'Привет, {user_list[1]}! Что пожелаешь?')
    else:
        bot.send_message(message.from_user.id, f'Напишите "Привет" или другое приветственное слово')
...
def callback_worker(call):
    if call.data == 'yes registraion':
        ...
        result,name = write_table_data(call.message.chat)
        if result:
            bot.send_message(call.message.chat.id,f'{name}, Вы успешно зарегистрированы!')
        else:
